...
func switch_scenes_animation():
	...
	tween.tween_property($Camera3D,"global_position",cam_pos,animation_time).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_IN_OUT)
	
	await get_tree().create_timer(rot_delay).timeout
	var cam_rot=Vector3(-30*PI/180,PI,0)
	var tween2=get_tree().create_tween()
	tween2.tween_property($Camera3D,"rotation",cam_rot,animation_time-rot_delay).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_IN_OUT)
